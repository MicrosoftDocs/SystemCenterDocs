...
---
# Syslog Events
Syslog events can be used to collect messages from Unix systems and other devices in [!INCLUDE[om12short](../../om/manage/includes/om12short_md.md)]. Syslog rules can be run on an agent that is the receiver of messages from one or more devices. When the rule is run, the agent will listen for messages on UDP port 514. This is the only port that can be used.  
...
|Target|The class to use for the target of the rule. The rule will be run on any agent that has at least one instance of this class. For more information on targets, see [Understanding Classes and Objects](../../om/manage/Understanding-Classes-and-Objects.md).|  
...
The following procedure shows how to create a Syslog event alerting rule in [!INCLUDE[om12short](../../om/manage/includes/om12short_md.md)] with the following details:  
