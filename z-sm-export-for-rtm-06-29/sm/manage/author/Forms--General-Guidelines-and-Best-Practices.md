...
You can extend features of [!INCLUDE[smlong12](../../../sm/deploy/deploy-guide/includes/smlong12_md.md)] by adding or modifying forms. This topic describes some best practice recommendations for creating and using [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] forms, using various tools and scripting form definitions directly.  
...
-   Use [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] forms infrastructure validation rules, value convertors, and error templates.  
...
### Include Exception Handling  
 Ensure that the code in the form contains exception handling so that the form can be loaded both during the design phase in the [!INCLUDE[scauthoringshort](../../../sm/manage/author/includes/scauthoringshort_md.md)] and in the [!INCLUDE[smcons](../../../sm/deploy/deploy-guide/includes/smcons_md.md)] at run time.  
...
### Bind the Form to Data Sources  
 The main purpose of a form is to visualize a single object from the [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] database. This object is called a **target instance**, which is always specified by the **DataContext** property of a form \(which is inherited from the **FrameworkElement** class\).  
...
>  Do not modify the form’s **DataContext** property. The forms hosting environment uses this property to identify the form target instance.  
 ...
 In the [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] data model, a target instance is represented as a **BindableDataItem** object. This class aggregates the underlying software development kit \(SDK\) object, and it exposes its properties through an indexer, which takes a property name as a parameter.  
