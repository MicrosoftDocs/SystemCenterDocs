...
## Custom MDX  
 You can use custom Multi\-Dimensional Expression \(MDX\) scripts to modify and tailor the OLAP cube to the exact specifications that meet your needs. Because [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] are model based, it is impossible to determine all your possible semantic needs when taking into account the wide spectrum of requirements and exact specifications for the domain\-specific business needs of a particular user. Custom MDX makes it possible for you to define MDX scripts that will be applied to the OLAP cube to enable specific scenarios that users need to measure and instrument.  
...
```  
 ...
 Remember that [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] supports only NamedCalculation targeting dimensions. NamedCalculation cannot target facts. The following table describes named calculation attributes.  
...
## Measure  
 You can use custom measures to aggregate and display data based on numeric attributes from dimensions. [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] does not support custom measures based on facts. Continuing with the example of he Named Calculation above, [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] defines a custom measure on IncidentsPastTargetResolutionTime as the following:  
...
```  
 ...
 Finally, the measure type is defined to be a sum. Possible values for a measure type include Sum and Count. Because of performance considerations, [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] Distinct Count measure types are not allowed. The following table describes measure attributes.  
...
## Action  
 Actions are events that you can trigger on an OLAP cube when you are accessing data in the cube. Only drill\-through actions are supported by [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)]. The following is an example of an action:  
...
|ActionType|Yes|\(DrillThrough\)|Type of action. Only drill\-through actions are supported by [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)].|  
