...
In [!INCLUDE[smlong12](../../../sm/deploy/deploy-guide/includes/smlong12_md.md)], you can contact a user by email or by instant message when an incident form is open. The presence indicator is shown in the form next to the affected user’s name, and it displays their current status, if known. For the presence indicator to accurately reflect a user’s status, the user must have an Active Directory account, and the user must be a member of the same domain in which the [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] management server has its computer account. Additionally, the computer running the [!INCLUDE[smcons](../../../sm/deploy/deploy-guide/includes/smcons_md.md)] must have Microsoft Office Lync 2010 installed.  
  
> [!NOTE]  
>  If a user’s account belongs to a domain other than the domain in which the [!INCLUDE[smshort](../../../sm/deploy/deploy-guide/includes/smshort_md.md)] management server has its computer account, the presence indicator might not accurately display the user’s status.  
