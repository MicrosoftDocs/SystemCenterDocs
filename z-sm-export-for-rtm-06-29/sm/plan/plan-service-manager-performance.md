...
Performance for System Center 2012 - Service Manager server roles and features is affected by different factors. Generally, there are three areas where positive and negative performance is most noticeable in Service Manager:  
...
-   In addition, when you create new workflows, such as a new notification subscription, additional load is placed on the system. As the number of new workflows that you create increases, the time it takes for each workflow to run also increases.  
 ...
 Workflow performance in System Center 2012 - Service Manager has improved from System Center Service ManagerÂ 2010 because the new ManagmentHostKeepAlive management pack has increased workflow processing responsiveness so that almost all workflows process within one minute.  
