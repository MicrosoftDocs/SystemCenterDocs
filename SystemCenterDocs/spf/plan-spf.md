...
-	Make sure you have the minimum [hardware and software](https://docs.microsoft.com/system-center/spf/system-requirements-spf) requirements on the SPF server.
-	The SPF server needs SQL Server for its database. The SQL Server database can be local, or on a remote server and should have at least 5 GB of storage. When you install SPF you need to specify the server name and port number. [Learn more](https://docs.microsoft.com/system-center/spf/system-requirements-spf#sql-server) about supported SQL Server versions.
