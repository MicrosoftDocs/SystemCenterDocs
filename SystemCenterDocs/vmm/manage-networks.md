...
5. [Create port profiles](network-port-profile.md): Create uplink port profiles that indicate to VMM which networks a host can connect to on a specific network adapter. If required, create virtual port profiles to specify settings that should be applied to virtual network adapters. You can create custom port classifications to abstract virtual port profiles.
