...
**Supported storage arrays** | Specific storage arrays are supported in VMM. You can use other arrays, but there's no guarantee that you'll be able to manage all storage tasks in VMM. We recommend that you chat to your storage provider to determine VMM support.
...
- **Storage replication**: VMM supports Windows Storage Replica for protecting data in a primary storage volume that replicating it to a secondary volume. [Learn more](storage-replica.md).
