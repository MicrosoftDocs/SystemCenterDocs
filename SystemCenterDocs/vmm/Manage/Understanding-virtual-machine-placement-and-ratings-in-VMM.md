...
When you deploy or migrate a virtual machine to a host, [!INCLUDE[vmm12sp1_long](../../includes/vmm12sp1_long_md.md)] uses virtual machine placement \(also known as intelligent placement\) to evaluate the suitability of available hosts. The placement algorithm analyzes performance data for the workload and the host, and then rates hosts on a scale of one to five stars to indicate the best placement choice. The [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] placement process for a number of deployment scenarios is used as summarized in the following table.

|Deployment scenario|Details|
|-----------------------|-----------|
|Create a new virtual machine from a disk, existing virtual machine, or template.|When you create a new virtual machine the host rating is used to offer suggestions for selecting a host.<br /><br />If a self\-service user creates a virtual machine, or you use drag\-and\-drop to move a virtual machine to a host group, the host will be assigned automatically by [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)], based on the highest rating.|
...
|Migrate a virtual machine|During migration \(offline, quick migration, live migration\), [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] provides host ratings to help you select appropriate hosts.|
...
Placement in [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] includes the following:

-   [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] displays hosts as they are rated, and you can select a suitable host before host rating is complete.

-   Placement on clusters includes the following choices:

    -   **Preferred owners:** You can specify preferred host owners for the virtual machine. This property is a Failover Cluster Manager setting which is used at the time of failover. When a virtual machine needs to be failed over, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] tries to use the preferred owners first.

    -   **Possible owners:** This feature allows you to specify possible host owners for a virtual machine.  If a host is not included in the possible owners, both [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] and Failover Cluster Manager will not allow the virtual machine to be placed on that host.
...
A host is rated only when a virtual machine needs to be placed. The information gathered about a host is compared to the information about the resources required by the virtual machine, and a rating is assigned to the host. During automatic placement, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] attempts to use the host assigned the highest rating. During manual placement the host rating is shown so that you can select the appropriate host. You can select a host in [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] even if not all hosts have been rated. The selected host must have a positive number of stars.

[!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] measures CPU, memory, disk, and network usage approximately every 10 minutes to recalculate an average rating that is an average of all the measurements taken that the last action that reset the host rating. Host ratings are reset when the following happens:
...
-   Highly available virtual machines must be placed on clustered hosts. [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] assigns zero stars to hosts that are not clustered but manual placement is not blocked. If you migrate a highly\-available virtual machine to a non\-clustered host, the virtual machine will no longer be highly available after the migration.

-   [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] blocks migration of Hyper\-V hosts to hosts running different virtualization software. Migration of a virtual machine with specific features not allowed by the virtualization software that is running on a host will be blocked. For example, Hyper\-V hosts do not allow booting up from a SCSI hard disk.
