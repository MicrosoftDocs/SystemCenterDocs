...
Use the following procedure to run Dynamic Optimization on demand on a host cluster in [!INCLUDE[vmm12sp1_long](../../includes/vmm12sp1_long_md.md)]. Through Dynamic Optimization, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] can balance load among hosts by migrating virtual machines within a host cluster. [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] only performs Dynamic Optimization on host clusters that support live migration. On demand Dynamic Optimization does not require that Dynamic Optimization be configured on the parent host group.
...
3.  On the **Folder** tab, in the **Optimization** group, click **Optimize Hosts**.
    ...
    [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] performs a Dynamic Optimization review to determine whether virtual machines can be migrated to improve load balancing in the host cluster. If migrating virtual machines can improve load balancing, VMM displays a list of virtual machines that are recommended for migration, with the current and target hosts indicated. The list excludes any hosts that are in maintenance mode in [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] and any virtual machines that are not highly available.
