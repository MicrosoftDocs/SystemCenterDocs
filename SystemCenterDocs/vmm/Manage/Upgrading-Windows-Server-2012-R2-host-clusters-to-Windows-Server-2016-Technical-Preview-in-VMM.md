...
VMM can manage the upgrade of Hyper\-V host clusters in the VMM fabric to [!INCLUDE[winthreshold_server_2](../../includes/winthreshold_server_2_md.md)]. You can upgrade the entire cluster, or select specific nodes to upgrade.
...
4.  Removes the node from the VMM console. This removes all [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] agents, virtual switch extensions, and so forth from the node.
...
7.  Adds the node back into the cluster, brings it out of maintenance mode, and returns the virtual machine workloads.
    ...
    > [!NOTE]
    > As soon as [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] upgrades one node, the cluster enters mixed mode. It continues to function as a [!INCLUDE[winblue_server_2](../../includes/winblue_server_2_md.md)] cluster.

When all of the nodes are running [!INCLUDE[winthreshold_server_2](../../includes/winthreshold_server_2_md.md)], [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] updates the cluster to a [!INCLUDE[winthreshold_server_2](../../includes/winthreshold_server_2_md.md)] cluster.

## Before you start 
Before you start the cluster upgrade, make sure of the following:

-   The cluster must be managed by [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)].

-   The cluster must meet the requirements for bare\-metal deployment , except that the physical computer profile does not need to include network or disk configuration details. During the upgrade process, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] records the node's actual network and disk configuration, and uses that information instead of the corresponding settings in the physical computer profile.
