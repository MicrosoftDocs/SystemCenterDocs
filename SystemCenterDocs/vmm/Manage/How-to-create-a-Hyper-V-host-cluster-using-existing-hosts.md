...
6. In **Storage** select the data disks you want the cluster to use. The list of available disks includes the logical units associated with the host group that you selected at the beginning of the wizard.
    ...
    -   If you assigned storage out\-of\-band, disks that are not managed by [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] are displayed and selected as available disks, with the check box next to each disk dimmed and unavailable.
