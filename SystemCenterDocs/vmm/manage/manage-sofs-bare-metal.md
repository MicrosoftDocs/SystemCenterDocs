...
In addition to adding existing file servers to the System Center 2016 - Virtual Machine Manager (VMM) storage fabric, VMM can discover physical bare-metal machines, automatically install an operating system, and provision them as SOFS cluster nodes. This article provides the steps for deploying a bare metal SOFS cluster.
...
**Physical computer** | Operating system | Computers must be running Windows Server 2012 R2 or later.
...
**Boot order** | Boot order | On each computer, set the BIOS boot order to boot from a Pre-Boot Execution Environment (PXE)-enabled network adapter as the first device.
...
**Accounts** | You'll need two RunAs accounts.<br/><br/> A Run As account for joining computers to the domain. You can create a Run As account in the Settings workspace.<br/><br/> A Run As account for access to the baseboard management controller (BMC) on each computer.

After you've verified the prerequisites, setting up an SOFS from bare-metal requires these steps:
