...
**Important:** VMM supports full lifecycle management of Nano Server-based VMs (including shielded VMs), however there are some known issues with respect to management of Nano VMs in VMM.
* You cannot create a VM template out of a Nano Server-based VM in VMM. Workaround is to create a VM template from scratch using a Nano Server VHD.
* Unlike Full Server VMs, there are some know issues in domain joining a VM in VMM. If you try to domain join a Nano Server-based VM by specifying the customization details in the VM Template, VMM does not honour it. The VM gets deployed but it does not join the specified domain. Workaround is to wait for the VM to get deployed and then use a domain join blob to join the VM to the domain. For more details, please see 'Joining Nano Server to a domain' section in [Getting Started with Nano Server](https://technet.microsoft.com/en-us/windows-server-docs/compute/nano-server/getting-started-with-nano-server)
