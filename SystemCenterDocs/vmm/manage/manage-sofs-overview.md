...
You can add and manage SOFS clusters in the System Center 2016 - Virtual Machine Manager (VMM) fabric. There are a number of ways you can add an SOFS cluster. You can add an existing SOFS cluster to the fabric,  you can provision an SOFS cluster from existing Windows machines in the fabric, or you can provision a cluster from bare metal computers.

- [Perform a rolling upgrade of an SOFS cluster](manage-sofs-rolling-upgrade.md)
- [Add an existing SOFS to the VMM storage fabric](manage-soft-add-existing-server.md)
- [Create an SOFS cluster from standalone servers in the VMM fabric](manage-sofs-create-standalone-server.md)
