...
2. [Create VM networks](manage-network-vm-networks): Create VM networks that map to network sites that exist within your physical networks.
...
4. [Add a gateway](manage-network-add-gateway.md): You might need to set up network virtualization gateways in the VMM networking fabric. By default, if you're using isolated VM networks in your VMM fabric, VMs associated with a network can only connect to machines in the same subnet. If you want to connect VMs further than the subnet you'll need a gateway.
