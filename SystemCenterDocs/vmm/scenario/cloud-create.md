...
## Before you start
  ...
  - You should have one or more [Hyper-V](../manage/manage-compute-hyper-v-overview.md) or [VMware](../manage/manage-compute-add-vmware.md) virtualization hosts in the fabric. If you're creating a cloud from a VMware resource pool,  a vCenter Server and the VMware ESX host or host cluster that contains the VMware resource pool must be available in the VMM fabric.
    - If you want to provide [self-service users](../manage/manage-self-service.md) the ability to store virtual machines to the VMM library, then create a library share, or create a folder in a library share that will serve as the storage location. [Learn more](../manage/manage-library-overview.md).
