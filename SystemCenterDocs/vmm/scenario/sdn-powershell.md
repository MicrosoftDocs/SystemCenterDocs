...
6. [Set up a dedicated VMM host group](sdn-network-controller.md#set-up-host-groups) for all SDN Hyper-V hosts. Note that hosts must be running the latest version of Windows Server 2016, and have the Hyper-V role enabled.
7. [Set up a certificate](sdn-network-controller.md#set-up-certificates). You need an SSL certificate for HTTPS communications between VMM and the network controller.
8. [Download](https://github.com/manishmsft/SDN/tree/master/VMM/VMM SDN Express) and run the SDN scripts. There are three scripts:
