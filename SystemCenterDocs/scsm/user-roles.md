...
#### Workflows
Workflows might have to read and write to the Service Manager database. During Setup, you are asked to provide credentials for the Workflows user role, and this user role performs the required actions on the Service Manager database. Like the user role, Report User, the narrow purpose of the Workflow user role means there's no reason for creating other user roles.
...
-   Sign in to the Service Manager console as one of the users assigned to the user role. Verify that you can't access data for which you do not have access rights, as specified in the user roles.
