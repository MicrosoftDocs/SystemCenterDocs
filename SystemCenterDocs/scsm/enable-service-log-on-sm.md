...
System Center 2019 - Service Manager (SM) supports hardening of service accounts and does not require granting the *Allow log on locally* user right for several accounts, required in support of SM.

You must provide service log on permission to the following accounts used by Service Manager Management Server and Service Manager Data Warehouse Management Server.
...
>[!NOTE]
> We recommend that you provide service logon permission to the accounts used by various SM connectors (AD, OM, SCO, CM, VMM, exchange connectors). >Service reporting account and analysis services accounts do not require service log on permission.
...
If required accounts are not provided service log on permission, then *monitoringhost.exe* does not run under those accounts. Which means, some of the workflows such as SLA/SLO would not run, and the following error event will be logged in the Operations Manager event log:
