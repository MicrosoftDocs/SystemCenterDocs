...
You can use the following procedure to create a virtual IP \(VIP\) template for Microsoft Network Load Balancing \(NLB\) in [!INCLUDE[vmm12sp1_long](../../includes/vmm12sp1_long_md.md)]. A VIP template contains load balancer\-related configuration settings for a specific type of network traffic. For example, you could create a template that specifies the load balancing behavior for HTTPS traffic on port 443.
...
8.  On the **Persistence** page, you can select the **Enable persistence** check box to enable session persistence \(also known as affinity\). If you enable persistence, the load balancer will always try to direct the same client to the same virtual machine that is behind the load balancer. This is based on the source IP address and the subnet mask.
    ...
    > [!NOTE]
    > When you deploy a service where a tier is configured to use NLB, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] automatically creates the NLB host cluster.
