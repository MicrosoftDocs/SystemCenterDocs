...
You can use [!INCLUDE[vmm12sp1_long](../../includes/vmm12sp1_long_md.md)] to deploy a host cluster, either to “bare\-metal computers” \(no operating system installed\), or computers with an installed operating system that will be overwritten during the process. For this deployment, [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] does the following:
...
4.  Brings the provisioned cluster under [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] management.
...
4.  On the **Home** tab, in the **Create** group, click **Create** and then click **Hyper\-V Cluster**. The wizard opens.

5.  On the **General Configuration** page, specify a name, for example, **HyperVClus02**, and the host group for the cluster, for example, **New York\\Tier0\_NY**.

    Then click **Next**.

6.  On the **Resource Type** page, select **Physical computers to be provisioned**, then fill in the options:

    1.  Specify the Administrator Run As account that [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] will use for creating the cluster.

    2.  Select the physical computer profile \(which will provide the domain name and administrator Run As account for each node\).

    3.  Next to the **BMC Run As account** box, click **Browse**, select a Run As account that has permissions to access the BMC, and then click **OK**. For example, if you had a Run As account called **BMC Administrator**, you could select that account.
    ...
    4.  In the **Out\-of\-band management protocol** list, click the protocol that your BMCs use.
    ...
    5.  For **Out of band management port**, ensure that the correct port is selected.

    6.  If the **Skip cluster validation** option appears, and you do not require support from Microsoft for this cluster, you can skip validation.
...
8.  If you specified a single IP address on the previous page, skip this step. Otherwise, the **Target Resources** page appears. Review the list of discovered BMCs \(identified by IP addresses\), and select the ones you want to include in the cluster.
    ...
    If you don't see all the BMCs that you expect, confirm that they are on a network accessible to the [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] server, and as needed, click **Refresh**. If BMCs still appear to be missing, review [Prerequisites: creating hosts or host clusters from bare metal with VMM](Prerequisites--creating-hosts-or-host-clusters-from-bare-metal-with-VMM.md).
...
9. On the **Deployment Customization** page, review the list of computers again, and provide information for each computer that you want to include. If you see a computer that you don't want to include, select the BMC \(identified by IP address\) and then click **Remove**.
    ...
    4.  For the computer you are configuring, click a network adapter \(on the left\). You can leave the displayed configuration unchanged, or fill in more information by clicking the button on the right, labeled either with **Configure** or with an ellipsis \(**...**\).
        ...
        -   **MAC address\(host clusters only—management NIC\)**: If this is the management NIC for a host cluster, and you want to configure it as a virtual network adapter, type a MAC address.
            ...
            > [!NOTE]
            > This is not the MAC address of the BMC. It is the MAC address of the management NIC, which can be used to communicate with the [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] management server.
...
10. On the **Summary** page, confirm the settings, and then click **Finish** to deploy the computers, create the cluster, and bring it under [!INCLUDE[vmm12short](../../includes/vmm12short_md.md)] management.
...
11. To confirm that the cluster was added:

    1.  Open the **Fabric** workspace.

    2.  In the **Fabric** pane, expand **Servers** > **All Hosts**, and then locate and click the new host cluster. In the **Hosts** pane, in the **Host Status** column, verify that each node in the cluster is **OK**.
