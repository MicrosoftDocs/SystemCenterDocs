...
Performance for [!INCLUDE[scsm_threshold_1](../Token/scsm_threshold_1_md.md)] server roles and features is affected by different factors. Generally, there are three areas where positive and negative performance is most noticeable in [!INCLUDE[smshort12](../Token/smshort12_md.md)]:

-   [!INCLUDE[smcons](../Token/smcons_md.md)] responsiveness. This is the length of time it takes from the moment you take some sort of action in the console until it completes.

-   Data insertion time for connectors. This is how long it takes for [!INCLUDE[smshort12](../Token/smshort12_md.md)] to import data when a connector synchronizes.
...
Connector initial synchronization can take a significant amount of time, for example, 8 to 12 hours for a large initial synchronization with System Center Configuration Manager. As a connector synchronizes initially, you can expect performance to suffer for all [!INCLUDE[smshort12](../Token/smshort12_md.md)] server roles and processes during this time. This occurs because of the way that data is inserted sequentially into the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database, which is a Microsoft SQL Server database. Although you cannot hasten the connector’s initial synchronization process, you can plan for the initial synchronization and ensure that the synchronization process completes well before [!INCLUDE[smshort12](../Token/smshort12_md.md)] is put into production.

When the initial synchronization is complete, [!INCLUDE[smshort12](../Token/smshort12_md.md)] continues synchronizing the differences, which does not have a measurable impact on performance.
...
-   Normally, workflows start and finish within one minute. When [!INCLUDE[smshort12](../Token/smshort12_md.md)] server roles are under a heavy workload, workflows do not complete as quickly as normal.
...
Workflow performance in [!INCLUDE[scsm_threshold_1](../Token/scsm_threshold_1_md.md)] has improved from [!INCLUDE[scsm_threshold_1](../Token/scsm_threshold_1_md.md)] because the new ManagmentHostKeepAlive management pack has increased workflow processing responsiveness so that almost all workflows process within one minute.
...
Often, administrators create groups to make sure that users have access to specified groups only. For example, in one scenario you might want to create a subset of incidents, such as incidents that affect computers that are used by human resource personnel. In this scenario, you might want only specific personnel to be able to view or modify the group of Sensitive Servers. Then, to enable this type for access, you would need to create a group for all users and a group for sensitive computers and then ensure that a security role has access to both the All Users and the Sensitive Servers groups. Inevitably, creating a group containing all users results in performance impact because [!INCLUDE[smshort12](../Token/smshort12_md.md)] frequently checks to determine if there are changes to the group. This check occurs once every 30 seconds, by default. For a very large group, checking for the changes creates a heavy load on the system, and it may slow down response time considerably.

Solution 1: You can manually specify how often [!INCLUDE[smshort12](../Token/smshort12_md.md)] checks for group changes by modifying a registry key. For example, if you change the group check frequency from 30 seconds to 10 minutes, you significantly increase performance. However, queues and service level objectives are special kinds of groups that use the same group calculation polling interval. So, increasing the value of the polling interval will result in longer times for queue and service level objective calculations.
...
Performance of the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database is directly affected by various factors, including the number of concurrent [!INCLUDE[smcons](../Token/smcons_md.md)]s that are reading or writing data, the group change check interval, and data that is inserted by connectors. More information is available in this document. Here are a few key points:

-   You should have a minimum of 8 gigabytes \(GB\) of RAM for the management server that hosts the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database in so that you can have an acceptable response time in typical scenarios.

-   You should have at least 8 CPU cores on the computer hosting the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database.

-   You can achieve better database performance by segregating log files and data files to separate physical disks, if possible. You can achieve further benefits by moving your tempdb to a different physical RAID drive than that of the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database. Use a RAID 1\+0 disk system to host your [!INCLUDE[smshort12](../Token/smshort12_md.md)] database, if possible.

-   Performance can be negatively affected if the [!INCLUDE[smshort12](../Token/smshort12_md.md)] database is created with a smaller size and it is set to autogrow, especially by small increments.

See the [!INCLUDE[smshort12](../Token/smshort12_md.md)] Sizing Helper tool that is included in the [Service Manager job aids](http://go.microsoft.com/fwlink/p/?LinkID=232378) documentation set \(SM\_job\_aids.zip\) for help in assessing the size of the database, and create the database with a size that is closer to the final size. This will help performance by reducing the amount of times the database has to autogrow.
...
Performance of the [!INCLUDE[smshort12](../Token/smshort12_md.md)] management server is primarily affected by the number of active concurrent [!INCLUDE[smcons](../Token/smcons_md.md)]s. Because all [!INCLUDE[smshort12](../Token/smshort12_md.md)] roles interact with the management server, consider adding additional management servers if you plan to have a large number of concurrent consoles. You should have 8 GB of RAM for the management server. You should have at least 4 CPU cores per management server, assuming that you have 10 to 12 active consoles per CPU core.

## Service Manager Console Performance
Performance of the [!INCLUDE[smcons](../Token/smcons_md.md)] is primarily affected by the number of forms that your analysts typically have open and the amount of data that is retrieved by views. You should have 4 GB of RAM on the computer where the [!INCLUDE[smcons](../Token/smcons_md.md)] is installed. If you have views that retrieve a large amount of data, you will need additional RAM. You should have at least a 4\-core CPU for the computer where the [!INCLUDE[smcons](../Token/smcons_md.md)] is installed. Because the [!INCLUDE[smcons](../Token/smcons_md.md)] is an end user application, we recommend that you restart it if you see excessive resource consumption. The [!INCLUDE[smcons](../Token/smcons_md.md)] aggressively caches information in memory, which can contribute to overall memory usage.

## Service Manager Data Warehouse Database Performance
Performance of the data warehouse is directly affected by various factors, including the number of concurrent [!INCLUDE[smshort12](../Token/smshort12_md.md)] management servers sending data, volume of data stored or the data retention period, rate of data change, and the extraction, transformation, and load \(ETL\) frequency. The amount of data that is stored in the data warehouse increases over time. Ensuring that you archive unnecessary data is important. Another factor that affects data warehouse performance is the BatchSize setting of ETL processes.
