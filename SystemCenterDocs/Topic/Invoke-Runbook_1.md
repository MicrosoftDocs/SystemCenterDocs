---
title: Invoke Runbook_1
ms.custom: na
ms.reviewer: na
ms.suite: na
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 76ffb95a-53d4-472c-8a18-9a1cebeeb697
---
# Invoke Runbook_1
The Invoke Runbook activity launches a runbook that you have specified. You can transfer data to runbooks by configuring an [Initialize Data](../Topic/Initialize-Data.md) activity in the invoked runbook. You can return data from the invoked runbook by configuring a [Return Data](../Topic/Return-Data.md) activity.
...
|**Parameters**|If you have selected a runbook that contains an [Initialize Data](../Topic/Initialize-Data.md) activity, the list of parameters required to invoke that activity will be displayed. Enter a value for each parameter.|
...
The Invoke Runbook activity returns any data that the invoked runbook has defined in the Returned Data tab of the runbook properties. The values of these properties must be populated using [Return Data](../Topic/Return-Data.md) activity in that workflow. If the current runbook needs to return data from the invoked runbook, then it must have its own [Return Data](../Topic/Return-Data.md) activity that includes these values.

### Credentials
If you use the Invoke Runbook activity and you use [Security Credentials](../Topic/Common-Activity-Properties.md#BKMK_SecurityCredentials), the account you use must be a member of the Orchestrator System group to run successfully.

## See Also
[Initialize Data](../Topic/Initialize-Data.md)
[Return Data](../Topic/Return-Data.md)
[Security Credentials](../Topic/Common-Activity-Properties.md#BKMK_SecurityCredentials)
