...
|**Available Storage**|Shows available storage, that is, storage logical units that are assigned to the host cluster but are not Cluster Shared Volumes \(CSV\).<br /><br />You can also do the following:<br /><br />-   Add and remove storage logical units that are managed by [!INCLUDE[vmm12short](../Token/vmm12short_md.md)].<br />-   Convert available storage to shared storage \(CSV\).<br /><br />For more information, see [How to configure storage on a Hyper-V host cluster in VMM](../Topic/How-to-configure-storage-on-a-Hyper-V-host-cluster-in-VMM.md).|
|**Shared Volumes**|Shows the shared volumes \(CSVs\) that are allocated to the host cluster. You can also do the following:<br /><br />-   Add and remove CSVs that are managed by [!INCLUDE[vmm12short](../Token/vmm12short_md.md)].<br />-   Convert CSVs to available \(non\-CSV\) storage.<br /><br />For more information, see [How to configure storage on a Hyper-V host cluster in VMM](../Topic/How-to-configure-storage-on-a-Hyper-V-host-cluster-in-VMM.md).|
