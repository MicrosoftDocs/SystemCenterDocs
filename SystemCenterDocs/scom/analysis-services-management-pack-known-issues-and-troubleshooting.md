...
|**Service State** monitor does not work on a clustered SSAS node.|The **Service State** monitor does not work when the SQL Server Analysis Services server is used as a cluster node.|No resolution.|
