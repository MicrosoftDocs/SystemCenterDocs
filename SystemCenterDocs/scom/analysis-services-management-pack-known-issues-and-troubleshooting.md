...
|The "Partition Discovery" may drops errors in the case the target SSAS instance is stopped|On the SSAS instance with discovered multidimensional database partitions and an instance service is stopped afterward, the module 'Microsoft.SQLServer.AnalysisServices.Windows.Module.Discovery.MultidimensionalPartitionPropertyCD' may drop an error message in the Event log with ID 4221 and source "SQL Server Analysis Services Discovery MP" with the following message: "The service MSSQLServerOLAPService is not running".|The fix of this issue will be provided in the next release of the SSAS Management Pack.|
