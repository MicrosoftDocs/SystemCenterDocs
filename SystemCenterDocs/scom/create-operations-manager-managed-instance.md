...
- Ensure you allow ports 1433 and 11000-11999 from the Microsoft Azure Monitor SCOM managed instance (preview).
...
- Ensure to create a Managed Service Identity (MSI), configure Microsoft Azure Monitor SCOM managed instance (preview) role-based access control (RBAC) and create and configure an SQL MI Instance. For more information, see \<link\>.
...
The Managed Service Identity provide an identity for applications to use when connecting to resources that support Azure Active Directory (Azure AD) authentication. For Microsoft Azure Monitor SCOM managed instance (preview), a Managed Identity will replace the traditional four System Center Operations Manager service accounts and it will be used to access the SQL MI database.
...
1. Under **Basics**, do the following:
    1. **Project details**:
        1. **Subscription**: Select the Azure subscription in which you want to create Microsoft Azure Monitor SCOM managed instance (preview).
        1. **Resource group**: Select the resource group in which you want to create Microsoft Azure Monitor SCOM managed instance (preview).
    1. **Instance details**:
        1. **Region**: Select the region in which you want to create Microsoft Azure Monitor SCOM managed instance (preview).
...
## Configure Microsoft Azure Monitor SCOM managed instance (preview) role-based access control (RBAC)
...
1. In the **Role**, select the desired role to add users: Microsoft Azure Monitor SCOM managed instance (preview) Contributor (Similar to System Center Operations Manager Administrator) or Microsoft Azure Monitor SCOM managed instance (preview) Reader (Similar to System Center Operations Manager Operator).
...
1. Select **Set Admin**, search for your MSI (the same MSI that you provided during the Microsoft Azure Monitor SCOM managed instance (preview) creation flow). You will find the Admin added to the SQL MI Instance.
...
>[!Note]
>- As Microsoft Azure Monitor SCOM managed instance (preview) can be created only in West Europe and West US regions, ensure that the VNet is in one of these regions.
...
1. Under **Basics**, do the following:
    1. **Project details**:
        1. **Subscription**: Select the Azure subscription in which you want to place the Microsoft Azure Monitor SCOM managed instance (preview).
        1. **Resource group**: Select the resource group in which you want to place the Microsoft Azure Monitor SCOM managed instance (preview). If you don't have a resource group in which you want to place the Microsoft Azure Monitor SCOM managed instance (preview), select **Create New** to create a new resource group and then place the instance. We recommend you have a new resource group exclusively for Microsoft Azure Monitor SCOM managed instance (preview).
    1. **Instance details**:
        1. **Aquila instance name**: Enter Microsoft Azure Monitor SCOM managed instance (preview) name as desired.
            >[!Note]
            >- Microsoft Azure Monitor SCOM managed instance (preview) name can have only alphanumaric characters and up to 10 characters long.
            >- Microsoft Azure Monitor SCOM managed instance (preview) is equivalent to System Center Operation Manager Management Group so choose a name accordingly.
        1. **Region**: Select the region that is near to you geographically so that latency between your agents and the Microsoft Azure Monitor SCOM managed instance (preview) is as low as possible.
    1. **Azure Hybrid Benefit**: Select **Yes** if you are using a Windows Server license for your existing servers. This license is only applicable for the Windows Servers that will be used while creating VMs for the Microsoft Azure Monitor SCOM managed instance (preview) and it won't apply to existing Windows Servers.
1. Select **Next**.
1. Under **Networking**, do the following:
    ...
    1. **Domain account**
        1. **Username**: Enter the username of the account you created in your active directory as part of the pre-requisites. Same account will be used to join the management servers (created as part of the Microsoft Azure Monitor SCOM managed instance (preview)) and SQL MI servers to your domain. For more information, see Domain Account.
        1. **Password**: Enter the password.
1. Select **Next**.
1. Under **Database inputs**, do the following:
    1. **SQL managed instance**:
        1. **Resource Name**: Select the SQL MI resource name for the instance that you would like to associate with this Microsoft Azure Monitor SCOM managed instance (preview). Only the SQL MI instance, which has given permissions to the Microsoft Azure Monitor SCOM managed instance (preview) should be used here. For more information, see SQL MI creation and permission.
...
> [Connect the Microsoft Azure Monitor SCOM managed instance (preview) to Ops console](connect-managed-instance-ops-console.md)