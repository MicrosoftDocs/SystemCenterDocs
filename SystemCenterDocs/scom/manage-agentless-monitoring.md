...
System Center Operations Manager can gather performance and availability data on a computer that doesn't have an agent installed by using a proxy agent that's installed on another computer. Use agentless monitoring of computers when it's impossible or undesirable to install an agent on a computer.  

An agentless-managed computer is a Windows-based computer that you discover by using the Operations console. You assign a management server or agent-managed computer to provide remote (proxy) agent functionality for the computers.  

Operations Manager manages agentless-managed computers as if an agent is installed on them. Not all management packs work in the agentless mode. For more information, see the documentation for the management packs you're running.  

> [!IMPORTANT]  
> Agentless management of a computer doesn't work if the agentless-managed computer and its proxy communicate through a firewall. A management server can't collect descriptions for events or publishers that are present on an agentless managed computer but aren't present on the proxy agent.  
...
> [!IMPORTANT]  
> An agentless-managed computer uses more resources on a management server than an agent-managed computer.  
...
1. Sign in to the Operations console with an account that's a member of the Operations Manager Administrators role for the management group.  
...
5. On the **Auto or Advanced?** page, complete the following steps:    
...
6. On the **Discovery Method** page, you can locate the computers that you want to manage by either scanning or browsing Active Directory Domain Services or entering the computer names.    
    ...
    If you want to scan, complete the following steps:  
...
7. Select **Next**, and on the **Administrator Account** page, complete one of the following steps:
    ...
    - Select **Other user account**, enter the **User name** and **Password**, and then select the **Domain** from the list. If the user name isn't a domain account, select **This is a local computer account, not a domain account**.  
        ...
        > [!IMPORTANT]  
        > The account must have administrative privileges on the targeted computers. If you select **This is a local computer account, not a domain account**, the management server action account is used to perform discovery.  

8. Select **Discover** to display the **Discovery Progress** page. The time it takes discovery to finish depends on many factors, such as the criteria you specified and the configuration of the environment.    

    > [!NOTE]  
    > Computers that are already managed by the management group won't be returned by the wizard.  

9. On the **Select Objects to Manage**  page, complete the following steps:  
...
12. Select **Close**. The computers appear in the **Administration** workspace under **Agentless Managed**.

## Configure a proxy for agentless monitoring

When you set up agentless monitoring of a computer, select a proxy for each agentless-managed computer. By configuring an agent as a proxy agent, you enable it to submit data on behalf of another source. A management group can serve as a proxy, but this role takes up system resources. Use an agent-managed computer as a proxy agent.  
...
> [!NOTE]  
> If you remove a proxy agent from management, its agentless systems are no longer managed.  

Both the agentless-managed system and its proxy need access to the managing server through any firewalls. For more information about interacting with firewalls, see [Configuring a firewall for Operations Manager](plan-security-config-firewall.md).  
...
1. Sign in to the computer by using an account that's a member of the Operations Manager Administrators role for the management group.  
...
You enable monitoring of applications, features, and services by importing management packs. For an explanation of what management packs do and how to import and manage them, see [What is an Operations Manager management pack](manage-overview-management-pack.md).
