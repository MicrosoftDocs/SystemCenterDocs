...
Gateway servers are used to enable agent-management of computers that are outside the Kerberos trust boundary of management groups, such as in a domain that isn't trusted, a DMZ, or a Workgroup. The gateway server acts as a proxy of sorts for agent-to-management server communication. Agents in domains that aren't trusted communicate with the gateway server, and the gateway server communicates with one or more management servers. Because communication between the gateway server and the management servers occurs over only one port (TCP 5723), that port is the only one that has to be opened on any intervening firewalls to enable management of multiple agent-managed computers, versus needing to have the same port opened across all agent servers. 
...
1. If the gateway server is in a domain that does not have a two-way transitive trust with its primary management server, or is in a workgroup, we will need to acquire certificates to be used for authentication on both the gateway and management server.
...
Before proceeding with gateway setup and installation, it is important to register and approve the intended gateway machine as a gateway first, otherwise when the services start, it may connect to the management group and be registered as an agent instead, leading to issues. These steps will be performed from a management server, preferrably your primary or "RMSE" server.

1. There is an executable included with the SCOM installation media called "Microsoft.EnterpriseManagement.GatewayApprovalTool.exe", which can be found in the install media under `..\SupportTools\amd64\`.
...
1. Use the command below to register the intended gateway as a gateway ensure to replace "managementServerFQDN" with the actual DNS entry for the management server, and "gatewayFQDN" with the DNS entry for the inteded gateway:
    ...
    > [!NOTE]
    > If you want to prevent the gateway server from initiating communication with a management server, include the */ManagementServerInitiatesConnection=True* parameter on the command as shown below. Otherwise by default communication will initiate from the gateway itself.
    >
    > ```cmd
...
1. If the approval is successful, you'll see `The approval of server <GatewayFQDN> completed successfully.`
...
Once the intended gatway server has been registered with the management group, it is time to install the role on the new gateway.
...
1. In the **Install** area, select the **Gateway management server** link (not the large "Install" link, it will be smaller towards the bottom).
...
1. On the **Gateway Action Account** page, select the **Local System** account option, unless you've created a domain-based or local computer-based gateway Action account. Select **Next**.
1. On the **Microsoft Update** page, optionally indicate if you want to use Microsoft Update, and select **Next**. (Typically this should be No.)
...
Gateway servers communicate with a primary management server, if this connection is lost, the gateway and any attached agents will go grey and not be monitored. If you have multiple management servers, we can prevent this by configuring management servers the gateway will failover to until the primary is available again. To configure a failovers:
