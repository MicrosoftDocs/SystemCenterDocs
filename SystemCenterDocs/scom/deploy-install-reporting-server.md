...
An installation of SQL Server Reporting Services used by Operations Manager **can't be shared with any other application** as changes are made to the base SSRS installation to accomodate user roles and authentication with SCOM. If any reports exist on the SSRS instance where the Operations Manager Reporting Services installer executes, all existing data and reports are overwritten.
