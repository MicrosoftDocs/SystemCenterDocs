...
You can move the System Center 2016 – Operations Manager Reporting server component to a new server, or reinstall the component on the original server.  
...
Verify the installation of your report server by performing the following steps:
...
2. Restore the original **ReportServer** and **ReportServerTempDB** databases for SQL Server Reporting services to preserve your custom reports, favorites, schedules from the original reporting services deployment.  
...
3. Verify that you can successfully run a report from the Operations console.
4. Ensure that the health state of all Management servers is **Healthy**.
