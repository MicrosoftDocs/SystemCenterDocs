...
|**Availability group rollup monitors** are in a warning state when secondary replicas are healthy|Availability Replica Data Synchronization, Availability Replicas Role, Availability Replicas Connection, Synchronous Replicas Data Synchronization rollups have become unhealthy according to their respective SQL Server policy health states and are causing false-negative alerts. |The fix for this issue will be able in the next SQL Server MP release.|
|**Custom performance rules and monitors** do not collect or display data|Custom workflows do not work and drop errors on the SQL Server agent machine in the event log like: "Could not load file or assembly Microsoft.SQLServer.CustomMonitoring.Module.ScomModules with PublicKeyToken=31bf3856ad364e35. Strong name validation failed. Exception from HRESULT: 0x8013141A".| Please, contact the [SQL MP's feedback](mailto:SQLMPsfeedback@microsoft.com) for the temporary workaround. The fix for this issue will be able in the next SQL Server MP release.|
