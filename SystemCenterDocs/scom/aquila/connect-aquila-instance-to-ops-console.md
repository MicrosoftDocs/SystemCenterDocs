---
ms.assetid: 
title: Connect the Aquila (preview) instance to Ops console
description: This article describes how to connect the Aquila (preview) instance to Ops console.
author: jyothisuri
ms.author: jsuri
manager: jsuri
ms.date: 10/04/2022
...
# Connect the Aquila (preview) instance to Ops console

Aquila (preview) is compatible with System Center Operations Manager 2019. [Download](https://www.microsoft.com/evalcenter/evaluate-system-center-2019) a trial version of System Center Operations Manager 2019 (available for 180 days), if you don't have the System Center Operations Manager 2019 executable files. Fill the personal details form and download SCOM_2019.exe

After you create the Aquila (preview) Instance in Azure, connect the instance to Ops Console to configure the workloads that need to be monitored.

## Connection of Aquila (preview) instance to Ops console

Follow the below steps to connect an Aquila (preview) instance to Ops Console:

1. Identify server to install Ops Console: Identify a server where you want to install the Ops Console. Don't perform this on an Aquila (preview) VM, but rather on a separate VM. The VM needs to be a Windows Server. This server can be on-premises or on Azure.
...
1. Connect Aquila (preview) to Ops Console: Log in to the Ops Console and select **Connect To Server**. Add the FQDN pointing to one of the two management servers. 
   >[!Note]
...
1. Connect Aquila (preview) to the Web Console: To connect to the Web Console, go to properties in the Aquila homepage, and copy the value in the *SCOM Web Console URL*. Paste that URL in any browser to open the web console.
