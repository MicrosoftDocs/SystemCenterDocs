---
ms.assetid: 
title: Connect the Aquila instance to Ops console
description: This article describes how to connect the Aquila instance to Ops console.
author: jyothisuri
ms.author: jsuri
manager: jsuri
ms.date: 10/04/2022
...
Log in to the Ops Console and select **Connect To Server**. Add the FQDN pointing to one of the two management servers. 
   >[!Note]
...
If you only have a few agents outside the trust boundary, you can use certificates to directly connect them to the Aquila instance without a gateway server. For more information, see [Install a gateway server](/scom/deploy-install-gateway-server?view=sc-om-2019&preserve-view=true). If you need to install a gateway server, identify a desired server, install the gateway server from the System Center Operations Manager executable file and follow the steps in the installation wizard.
...
Install your agents using the [instructions](/system-center/scom/manage-deploy-windows-agent-console?view=sc-om-2019&preserve-view=true).
