---
ms.assetid: 
title: Azure Monitor SCOM Managed Instance Agents
description: This article provides design guidance for agent deployment on Windows  with Azure Monitor SCOM Managed Instance.
...
SCOM Managed Instance agents can be installed by using manual installation method. The setup is manually run on the agent or deployed through an existing software distribution tool. 

>[!NOTE]
>You cannot install MMA on a computer where SCOM Managed Instance management server, Operations Manager management server, System Center Operations Manager gateway server, System Center Essentials, or System Center Service Manager is installed - as they have their built-in version of MMA installed.
