...
SQL Server allows you to configure the minimum and maximum amount of memory that should be reserved and used by its process. It is recommended to specify at least 4 GB of RAM as minimum amount. This should be done for every SQL node hosting one of the Operations Manager databases (Operational, Data warehouse, ACS).
