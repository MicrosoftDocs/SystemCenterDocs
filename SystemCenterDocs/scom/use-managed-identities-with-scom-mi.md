...
*Managed identities for Azure* solve this problem for all your resources in Microsoft Entra ID by providing them with automatically managed identities. You can use a service's identity to authenticate any service that supports Microsoft Entra ID authentication, including Key vault, without sorting any credentials in your code.
...
- **Client ID** - A unique identifier generated by Microsoft Entra ID that is tied to an application and service principal during its initial provisioning. For more information, see   [Application (client) ID](/azure/active-directory/develop/developer-glossary#application-client-id).
