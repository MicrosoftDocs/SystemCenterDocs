...
>[!NOTE]
>MSOLEDB19 Driver is used to establish encrypted connections to the SQL Server (by default). If the SQL server certificate isn't **Trusted** on the Orchestrator machine,  enter *Server=\<serverName\>;Trust Server Certificate=True* for every configuration. [Learn more](/system-center/orchestrator/install?view=sc-orch-2025&preserve-view=true#secure-connection-to-sql-server) to install a SQL Server certificate.
