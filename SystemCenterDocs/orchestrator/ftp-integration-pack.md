...
15.  In the **HTTP Proxy Username (FTP)** and **HTTP Proxy Password (FTP)**, enter the credentials that Orchestrator will used to connect to the HTTP proxy server. This configuration property applies to FTP only. This configuration property is optional and can be left empty.
