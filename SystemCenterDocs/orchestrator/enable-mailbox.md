...
| Shared   | Specifies the new mailbox as a shared mailbox. A shared mailbox is a mailbox to which multiple users can sign in. A shared mailbox isn't associated with any of the users that can sign in. It is associated with a disabled user account. <br>Default is True, when selected. <br><br><strong>Note </strong><br> This property is available only in an on-premises environment. <br><br>   | True, False   |
