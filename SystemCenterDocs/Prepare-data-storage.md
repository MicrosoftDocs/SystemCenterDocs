...
|Dedup support|DPM running as a Hyper\-V virtual machine can store backup data to VHDs in shared folders on a Windows File Server with data deduplication enabled. For more information, see [Deduplicate DPM storage](../Topic/Deduplicate-DPM-storage.md).|
...
-   You'll need to attach your tape libraries or standalone tape drives to the DPM server with SAN or SCSI. Tape devices must be compatible with DPM. Read about [Identify compatible tape libraries](../Topic/Identify-compatible-tape-libraries.md) .
...
3.  Add firewall exceptions—Add firewall exceptions so that DPM can detect the tape:
    C:\\Program Files\\Microsoft System Center 2012\\DPM\\SQL\\SSQL10\_50.MSDPMV4RC\\MSSQL\\Binn\\sqlservr.exe
