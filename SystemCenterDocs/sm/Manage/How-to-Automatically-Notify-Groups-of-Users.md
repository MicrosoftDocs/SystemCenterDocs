...
6.  Wait for [!INCLUDE[smshort](../../includes/smshort_md.md)] to sync with Active Directory Domain Services \(AD DS\), or perform a manual Synchronization from **Administration**>**Connectors**. \(Click **AD Connector**, and then click the **Synchronize Now** task on the right\-hand side\).

7.  Once the Active Directory synchronization has completed, the newly created group will be available as a configuration item in [!INCLUDE[smshort](../../includes/smshort_md.md)], and it can be selected in the user picker fields, such as **Affected User** and **Assigned To**.
