...
Data warehouse module deployment in Service Manager starts when a [!INCLUDE[smshort](../../includes/smshort_md.md)] management server is registered to a data warehouse management server. The following sections describe module parts, functions, and schedule.
...
1.  Removing management packs does not delete the data from the data warehouse as it does in the  [!INCLUDE[smshort](../../includes/smshort_md.md)] database; instead, the database view that users are granted access to is dropped.
...
2.  If you reimport a management pack after you have removed the corresponding management pack, the historical data is exposed once again.
    ...
    > [!NOTE]
    > Only sealed management packs are synchronized from [!INCLUDE[smshort](../../includes/smshort_md.md)] to the data warehouse. An exception to this is list items, also known as enumerations. Groups or queues are synchronized to the data warehouse, regardless of whether they are in a sealed or unsealed management pack.

Management packs that are imported from [!INCLUDE[smshort](../../includes/smshort_md.md)] are [!INCLUDE[smshort](../../includes/smshort_md.md)]–specific and data warehouse specific. The [!INCLUDE[smshort](../../includes/smshort_md.md)] management packs provide awareness of what the [!INCLUDE[smshort](../../includes/smshort_md.md)] database is structured like, and the data warehouse management packs drive the structure and processes of the data warehouse databases.

## Report Deployment
The management pack synchronization process imports management packs from [!INCLUDE[smshort](../../includes/smshort_md.md)], and it defines how those management packs shape the structure, move the data, and copy reports for the data warehouse and reporting. After those management packs are synchronized between [!INCLUDE[smshort](../../includes/smshort_md.md)] and the data warehouse, the data is retrieved and reports are deployed for user consumption.
...
Management packs that contain only [!INCLUDE[smshort](../../includes/smshort_md.md)]–specific information do not cause the deployment activities to execute. They are only be triggered for new data warehouse and reporting\-specific elements.
...
### Extract
The extract process starts on a scheduled interval. Extract is the process that retrieves raw data from your online transaction processing system \(OLTP\) store, which in this case is the  [!INCLUDE[smshort](../../includes/smshort_md.md)] database.

1.  The extract process queries [!INCLUDE[smshort](../../includes/smshort_md.md)] for the delta data that has accumulated since the last time the extract process ran.

2.  The new data is written into the DWStagingandConfig database in the same basic form as it is in the [!INCLUDE[smshort](../../includes/smshort_md.md)] database.
