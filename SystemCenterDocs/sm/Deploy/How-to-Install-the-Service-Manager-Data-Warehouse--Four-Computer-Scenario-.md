...
8.  On the **Configure the data warehouse databases** page, click **Staging and Configuration**. In the **Database server** box, type the computer name of the computer that will host the two data warehouse databases. For example, type **Computer 4**, and then press the TAB key. Verify that **Default** appears in the **SQL Server instance** box.
    ...
    > [!IMPORTANT]
    > A warning message appears if you are using the default collation (SQL_Latin1_General_CP1_CI_AS). Support for multiple languages in Service Manager is not possible when you are using the default collation. If later you decide to support multiple languages using a different collation, you have to reinstall SQL Server. See â€œMicrosoft SQL Server 2008 with SP1â€ in the [Planning Guide for System Center 2012 â€" Service Manager](http://go.microsoft.com/fwlink/p/?LinkID=209672).
...
12. On the **Configure the reporting server for the data warehouse** page, follow these steps:
    ...
    3.  Because you followed the procedure â€œManual Steps to Configure the Remote SQL Server Reporting Servicesâ€ in the [Deployment Guide for System Center 2012 â€" Service Manager](http://go.microsoft.com/fwlink/p/?LinkID=209670) (http://go.microsoft.com/fwlink/p/?LinkID=209670), select the **I have taken the manual steps to configure the remote SQL Server Reporting Services as described in the Service Manager Deployment Guide** check box, and then click **Next**.
