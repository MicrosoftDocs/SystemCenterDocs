...
You can use MOMAgent.msi to deploy [!INCLUDE[scom_threshold_1](../../includes/scom_threshold_1_md.md)] agents from the command line. Deploying agents from the command line is also referred to as a manual install.
...
3.  Run the following command:
    ...
    |NOAPM\=1|Optional parameter. Installs the Operations Manager agent without .NET Application Performance Monitoring. If you are using [!INCLUDE[avicode57](../../includes/avicode57_md.md)], NOAPM\=1 leaves the AVIcode agent in place. If you are using [!INCLUDE[avicode57](../../includes/avicode57_md.md)] and install the Operations Manager agent by using momagent.msi without NOAPM\=1, the AVIcode agent will not work correctly and an alert will be generated.|
