...
|Unable to kill the MonitoringHost(s) on the Nano Server.|The SCOM Agent runs in the MonitoringHost process. If that process is active the Uninstallation script will not be able to terminate it.|Make sure the MonitoringHost process is not running, and run the uninstall script again.|
...
|Unable to remove the agent directory. Try restarting the Nano Server and then re-running this script.|A process may be using the SCOM Agent.|Make sure there are no processes attached to the SCOM agent and run the uninstall script again.|
