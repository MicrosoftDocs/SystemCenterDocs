...
* Update Rollup 1 is installed on SC DPM 2019.
  ...
  > [!NOTE]
  > With DPM 2019 UR1 and later, the offline seeding authenticates using Microsoft Entra ID.
...
* Ensure that you've the [necessary permissions](/azure/active-directory/develop/howto-create-service-principal-portal) to create the Microsoft Entra ID application. The Offline Backup workflow creates an Microsoft Entra ID application in the subscription associated with the Azure Storage account. The goal of the application is to provide Azure Backup with secure and scoped access to the Azure Import Service required for the Offline Backup workflow.
