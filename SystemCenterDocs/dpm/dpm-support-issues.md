...
**Workaround**: Virtual tape libraries configured with a virtual fiber channel adapter are supported on DPM 2019 or later with certified hardware. For the current list of supported hardware, see [Compatible Tape Libraries](./dpm/dpm-compatible-tape-libraries.md). To check if your tape library is supported by the virtual fiber channel adapter, contact your tape hardware vendor and ask them to [verify tape library compatibility](./dpm/identify-compatible-tape-libraries.md).
...
::: moniker range="<=sc-dpm-2019"
Protection of SQL Server database stored on Cluster Shared Volume (CSV) is supported with [DPM 2019 UR2](what-s-new-in-dpm.md) and later.
::: moniker-end
