...
- Data Protection Manager (MARS Agent) can directly write backup data onto the supported SKUs of Azure Data Box. This capability eliminates the need for you to provision a staging location for your initial backup data. You also don't need utilities to format and copy that data onto the disks.
...
3. On the **Select data protection method** page, specify how you want to handle short and long-term backup. Make sure you select **I want online protection.**
   ...
   ![Create new protection group](./media/offline-seeding-azure-data-box/create-new-protection-group.png)
...
8. On the **Specify online protection data** page, select the member you want to enable online protection.
    ...
    ![Specify online protection data](./media/offline-seeding-azure-data-box/specify-online-protection-data.png)
...
11. On the **Choose online replication** screen of the wizard, choose the option **Transfer using Microsoft owned disks** and select **Next**.
    ...
    ![Choose initial online replication](./media/offline-seeding-azure-data-box/choose-initial-online-replication.png)
...
12. Sign into Azure when prompted, using the user credentials that have owner access on the Azure Subscription. After a successful sign-in, the following screen is displayed:
    ...
    ![After successful login](./media/offline-seeding-azure-data-box/after-successful-login.png)
...
13. Select the correct Data box order for which you've unpacked, connected, and unlocked your Data Box disk. Select **Next**.
    ...
    ![Select the databox](./media/offline-seeding-azure-data-box/select-databox.png)

14. On the **Detect the DataBox** screen, enter the path of your Data Box device, and then select **Detect Device**.

    ![Enter network path](./media/offline-seeding-azure-data-box/enter-network-path.png)
...
15. Select **Next**. On the **Summary** page, review your settings and select **Create Group**.
    ...
    ![Detect databox](./media/offline-seeding-azure-data-box/detect-databox.png)

    The following screen confirms that the Protection Group is created successfully.

    ![Protection group created](./media/offline-seeding-azure-data-box/protection-group-created.png)
...
Data Protection Manager agent on the DPM server creates a Microsoft Entra application for you, in your tenant. This application requires a certificate for authentication that's created and uploaded when configuring offline seeding policy.
...
:::image type="content" source="./media/offline-seeding-azure-data-box/azure-recovery-services-agent.png" alt-text="Azure recovery services agent.":::
...
:::image type="content" source="./media/offline-seeding-azure-data-box/azure-recovery-services-agent-error-screen.png" alt-text="Screenshot of error screen for Azure recovery services agent.":::
...
2. If no other server has offline seeding configured and no other server is dependent on the `AzureOfflineBackup_<Azure User Id>` application, then delete this application from **Azure portal > Microsoft Entra ID > App registrations**.
   ...
   >
   > ![Public keys](./media/offline-seeding-azure-data-box/public-keys.png)
...
3. Go to the Azure Offline Backup application mentioned in **point 2**. In the **Settings** > **Keys** > **Upload Public Key,** upload the certificate exported in the step above.
   ...
   ![Upload public keys](./media/offline-seeding-azure-data-box/upload-public-keys.png)
