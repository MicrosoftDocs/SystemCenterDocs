...
description: This article provides information about how to deploy and manage the update rollups in System Center Data Protection Manager.
author: Jeronika-MS
...
>[!NOTE]
> Update rollups install automatically if Microsoft Updates are set to *Automatic*. If these updates aren't installed automatically, you can install them manually.
...
To manually install an update rollup package through an elevated command prompt, complete the following steps:
...
1. From the extracted location, run the following command to install the update rollup:
    ...
    >[!NOTE]
    > There might be additional installation steps specific to a UR. Check the relevant KB article to ensure you complete all the installation steps.
...
1. Go to **Control Panel** > **Programs and Features** > **View installed updates**.

2. Verify that an update entry exists for the update rollup.

   For example, the 2019 update rollup 2 was released as 4563392. You should see this detail under **View installed updates** if you installed this update rollup and if it was successfully installed.

3. Verify that the binary's version has the correct build number.  

4. Check the build number for a specific update rollup in the [list of build numbers for DPM](release-build-versions.md).

> [!NOTE]
> Not all the binaries have the current update rollup build number. However, if you don't see the binaries listed with the relevant update rollup build number, the update rollup likely didn't install successfully.
