...
- **Intuitive Email notifications** - support for HTML format notification. [Learn more](../scom/manage-notifications-create-email-channel.md#create-an-email-notification-in-html-format).
...
The existing URL monitoring capability has been enhanced. With this change, this capability will not ignore the server certificate errors (like server certificate CN, expiry date, untrusted CA and wrong usage) by default. If you wish to monitor a website, which does not have a valid SSL certificate, you must change the default settings to *ignore the server certificate errors*.
