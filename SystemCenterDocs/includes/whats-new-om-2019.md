...
Scheduled maintenance mode feature is in support by  Operations Manager 2016 and later. In case of Operations Manager deployments that had SQL **Always ON** enabled for high-availability, the schedule maintenance mode capability does not work as expected when the SQL Server failover occurs. Schedules that were created with *SQL Always ON* enabled were inaccessible, when SQL server fails over to another server.

Operations Manager 2019 comes with a fix for this issue. With this fix, scheduled maintenance mode works as expected, even in a SQL Server failover scenario.

Here are the details about the fix:  

* Schedules that were created before to this fix will also work without any issues even in SQL Server failover scenarios.
...
## New operating system support for monitoring
