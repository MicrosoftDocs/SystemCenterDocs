...
If you want to use the log file monitoring feature, you must install the Linux log file monitoring management pack that's provided in Operations Manager 2019. This change ensures that the *omsagent* user is created only when you use the log file monitoring feature. For more information, see [Install agent on UNIX and Linux computers](../scom/manage-install-crossplat-agent-cmdline.md).
