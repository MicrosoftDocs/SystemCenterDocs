...
When you receive an alert, from the alert details, you can see whether the alert was generated by a rule or a monitor. If the alert was generated by a monitor, as a best practice, you should allow the monitor to autoresolve the alert when the health state returns to healthy.
...
This behavior, which often led to closure of critical alerts without resolving the underlying problem is fixed with Operations Manager 2019. An alert, which is generated by a monitor cannot be closed unless the health state of the corresponding monitor is healthy. [Learn more](../scom/enhanced-alert-closure-experience.md)
...
With Operations Manager 2019, there are changes in the Linux agent package bundling. This bundle now consists of **scx** and **omi** shell bundles only. Post agent installation, a new user called **omiuser** will be created on the agent computer. If you want to use the Log Monitoring feature, your must install Linux Log Monitoring management pack, provided in Operations Manager 2019.
...
Scheduled maintenance mode has been a feature of Operations Manager since the 2016 release. In earlier releases, in case of Operations Manager deployments that had SQL Always ON enabled for high-availability, the schedules were inaccessible when the SQL Server failover occurs.

Operations Manager 2019 comes with a fix for this issue. With this fix, scheduled maintenance mode capability works as expected, even in case of SQL Server failover.
...
For rest of the requirements, please see Operations Manager 2019 system requirements.
...
The existing URL monitoring capability has been enhanced. With this improvement, this capability will not ignore the server certificate errors (like server certificate CN, expiry date, untrusted CA, and wrong usage) by default. If you wish to monitor a website, which does not have a valid SSL certificate, clear the *ignore the server certificate errors* checkbox in your Web application *Properties**,.
