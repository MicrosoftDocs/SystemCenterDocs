...
7.  On the **Credentials** page, either click **Browse** and then on the **Select a Run As Account** dialog box, specify the account described in the previous [Prerequisites](../Topic/VMM-networking-reference--using-an-IPAM-Server-in-VMM.md#BKMK_prereq) section, or click **Create Run As Account** and create a new Run As account with the permissions that are listed in the [Prerequisites](../Topic/VMM-networking-reference--using-an-IPAM-Server-in-VMM.md#BKMK_prereq) section. After you specify the account, click **Next**.
...
-   If you cannot control the time synchrony of the IPAM server and the [!INCLUDE[vmm12short](../Token/vmm12short_md.md)] server as described in the [Prerequisites](../Topic/VMM-networking-reference--using-an-IPAM-Server-in-VMM.md#BKMK_prereq) in this topic, you must update permissions on the IPAM server so that the provider software \(included in [!INCLUDE[vmm12short](../Token/vmm12short_md.md)]\) can query the current time setting on the IPAM server. To do this, on the IPAM server, run **wmimgmt.msc** to open the **WMI Control \(Local\)** snap\-in. Right\-click **WMI Control \(Local\)**, click **Properties**, and then click the **Security** tab. Navigate to **Root\\CIMV2**, click the **Security** button, select the account that you created for the [Prerequisites](../Topic/VMM-networking-reference--using-an-IPAM-Server-in-VMM.md#BKMK_prereq) in this topic, and then for **Remote Enable**, select the **Allow** box.
