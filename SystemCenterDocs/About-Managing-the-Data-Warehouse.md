...
|----------------------|---------------|
|MPSyncJob|This job synchronizes all the management packs from the [!INCLUDE[smshort]./Token/smshort_md.md)] source. These management packs define the content of the data warehouse. This job starts to run as soon as you register the [!INCLUDE[smshort]./Token/smshort_md.md)] management group, and it takes several hours to complete on its initial run.|
...
|Entity \(or Grooming\)|Grooming functions typically involve activities on the data warehouse that remove data based on a configurable time period. **Note:** For this release of [!INCLUDE[smshort]./Token/smshort_md.md)], grooming functions are handled as a workflow. Settings for this job are not configurable.|
|Extract|This job retrieves data from the [!INCLUDE[smshort]./Token/smshort_md.md)] database. This job queries the [!INCLUDE[smshort]./Token/smshort_md.md)] database for the delta data from its last run and writes this new data into the DWStagingAndConfig database in the data warehouse. There are two extract jobs in [!INCLUDE[smshort]./Token/smshort_md.md)]: one for the [!INCLUDE[smshort]./Token/smshort_md.md)] management group and the other for the data warehouse management group.|
...
In this release of [!INCLUDE[smshort]./Token/smshort_md.md)], grooming functions are handled as a workflow. Settings for this job are not configurable.

## Windows PowerShell Cmdlets
The [!INCLUDE[smshort]./Token/smshort_md.md)] Windows PowerShell module contains cmdlets that are used in this scenario to manage data warehouse functions on the server that hosts the data warehouse. You must run all Windows PowerShell cmdlets as an administrator. To view the Windows PowerShell Help, type the **get\-help** command, followed by the name of the cmdlet for which you want help. For example, type `get-help Set-SCDWJobSchedule`. The following cmdlets are used in this scenario:

-   **Get\-SCDWJobSchedule** —Displays the schedule for a data warehouse job.

-   **Get\-SCDWJob**—Displays status for all recurring [!INCLUDE[smshort]./Token/smshort_md.md)] data warehouse jobs.
