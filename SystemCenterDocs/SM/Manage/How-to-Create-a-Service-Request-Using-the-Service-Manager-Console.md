...
End users often create service requests in [!INCLUDE[scsm_threshold_1](../../includes/scsm_threshold_1_md.md)] by accessing the service catalog from the [!INCLUDE[smssp](../../includes/smssp_md.md)] or by submitting email requests. However, you can use the following procedure to manually create a new service request in the [!INCLUDE[smcons](../../includes/smcons_md.md)]. For example, you might want to manually create a new service request if a user contacts the help desk by telephone. In the following example, you can update any information that you want to as you complete the form.

### To create a new service request using the [!INCLUDE[smcons](../../includes/smcons_md.md)]

1.  In the [!INCLUDE[smcons](../../includes/smcons_md.md)], expand **Work Items**, expand **Service Request Fulfillment**, and then select a subnode, such as **Assigned To Me**.
